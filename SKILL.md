...
- Create a workspace API key in portal `Dashboard Keys` before using hosted MCP or gateway access.
...
- Hosted MCP and gateway now also expose live minimal symbol-first CEX market-read support through canonical `asset_profile`, `market_profile`, `price_snapshot`, and `ohlcv_window` queries.
